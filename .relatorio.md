...
Em casos específicos onde o número de rotas é maior do que o produto das máquinas que compõem rotas de entrada ou saída, ou seja:

$$
\begin{align*}
    |R_{x}| &> |\text{CD}| \times |\text{CBI}| \times |\text{SRI}| \\
    |R_{y}| &> |\text{SRO}| \times |\text{CBO}| \times |\text{SL}|
\end{align*}
$$

Algumas rotas podem compartilhar exatamente os mesmos caminhos, mas conectadas à subáreas distintas. A geração de instâncias do João não considerou esse caso, que é um caso muito atípico, mas, que ainda assim deve ser respeitado.
...
Vamos analisar um excerto da matriz de conflitos de rotas para um cenário de 10 rotas de entrada num porto com 2 viradores ($\text{CDI}$), 2 esteiras ($\text{CBI}$) e 2 empilhadeiras ($\text{SRI}$).
...
O joão faz esse cálculo de rotas conflitantes à partir de uma matriz de uso de equipamentos por rotas, mas, ela é vulnerável aos casos que eu falei no penúltimo slide, ela é codificada a para ter $|M_{x}|$ equipamentos linhas e $\frac{|M_{x}|}{2}$ colunas. Então, existem casos de rotas que não serão consideradas conflitantes, mas, que os são e ainda mais, existem casos de rotas faltando equipamentos (`RX_009`) por exemplo ou sem quaisquer equipamentos vinculados: (`RX_010`). Segue um excerto da matriz reproduzido exatamente como na instância do João:
...
Com isso, o conjunto de rotas conflitantes é inconsistente com a realidade operacional do problema. Reproduzindo esta mesma instância na minha formulação, o cojunto abaixo representa a diferença entre os meus conjuntos e os do João, que não foram considerados na implementação dele:
